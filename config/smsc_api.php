<?	
	...
	function send_sms($phones, $message, $translit = 0, $time = 0, $id = 0, $format = 0, $sender = false, $query = "", $files = array()) {
		...
		if (SMSC_DEBUG) {
			if ($m[1] > 0) echo "��������� ���������� �������. ID: $m[0], ����� SMS: $m[1], ���������: $m[2], ������: $m[3].\n";
			else echo "������ �", -$m[1], $m[0] ? ", ID: ".$m[0] : "", "\n";
		}
		return $m;
	}
	...
	function get_status($id, $phone, $all = 0) {
		...
		if (!strpos($id, ",")) {
			if (SMSC_DEBUG )
				if ($m[1] != "" && $m[1] >= 0) echo "������ SMS = $m[0]", $m[1] ? ", ����� ��������� ������� - ".date("d.m.Y H:i:s", $m[1]) : "", "\n";
				else echo "������ �", -$m[1], "\n";
			...
			if (count($m) == 1 && strpos($m[0], "-") == 2) return explode(",", $m[0]);
	...
	function _smsc_send_cmd($cmd, $arg = "", $files = array()) {
		...
		$i = 0;
		do {
			if ($i++) $url = str_replace('://smsc.kz/', '://www'.$i.'.smsc.kz/', $_url);
			$ret = _smsc_read_url($url, $files, 3 + $i);
		}
		while ($ret == "" && $i < 5);

		if ($ret == "") {
			if (SMSC_DEBUG) echo "������ ������ ������: $url\n";
			$ret = ","; // ��������� �����
		}
